...
export default function ResultPortal({ schoolName, schoolAddress }: ResultPortalProps) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setError("");
    setResult(null);
    if (!symbolNumber.trim() || !dob.trim()) { setError(t.results.enterBothFields); return; }
    setLoading(true);
    try {
      ...
      if (found) setResult(found); else setError(t.results.noResultFound);
    } catch { setError(t.results.errorFetching); }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-2xl shadow-lg border border-border p-6 lg:p-8 mb-8">
        <SectionHeading title={t.sections.ResultPortal} />

        <form onSubmit={handleSearch} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">{t.results.symbolNumber}</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">{t.results.dateOfBirth}</label>
          ...
          <button type="submit" disabled={loading} className="w-full sm:w-auto px-8 py-2.5 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-primary-dark transition-colors shadow-sm hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed inline-flex items-center gap-2">
            {loading ? <><Spinner /> {t.results.searchForResult}</> : t.results.viewResult}
          </button>
        </form>
      </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl border-2 border-primary overflow-hidden">
              <div className="bg-primary px-6 lg:px-10 py-6 text-center">
                ...
                <div className="mt-4 inline-block bg-secondary/20 border border-secondary/40 rounded-lg px-6 py-2">
                  <p className="text-secondary font-heading font-bold text-lg">{t.results.academicMarksheet}</p>
              ...
              <div className="px-6 lg:px-10 py-5 border-b border-border bg-surface">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                  <div><span className="text-muted text-xs block">{t.results.studentName}</span><strong>{result.studentName}</strong></div>
                  <div><span className="text-muted text-xs block">{t.results.class}</span><strong>{result.class}</strong></div>
                  <div><span className="text-muted text-xs block">{t.results.symbolNumberLabel}</span><strong>{result.symbolNumber}</strong></div>
                  <div><span className="text-muted text-xs block">{t.results.dateOfBirthLabel}</span><strong>{result.dob}</strong></div>
                </div>
              </div>

              <div className="px-4 lg:px-8 py-6">
                <h3 className="text-sm font-heading font-bold text-primary uppercase tracking-wider mb-4">{t.results.subjectWisePerformance}</h3>
                <div className="overflow-x-auto rounded-lg border border-border">
                  <table className="w-full text-sm">
                    <thead><tr className="bg-primary text-white"><th className="p-3 text-left font-semibold rounded-tl-lg">{t.results.subject}</th><th className="p-3 text-center font-semibold">{t.results.fullMarks}</th><th className="p-3 text-center font-semibold">{t.results.passMarks}</th><th className="p-3 text-center font-semibold">{t.results.obtained}</th><th className="p-3 text-center font-semibold rounded-tr-lg">{t.results.result}</th></tr></thead>
                    <tbody>
              ...
              <div className="px-4 lg:px-8 pb-6">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  <div className="bg-surface rounded-xl p-4 text-center border border-border"><p className="text-xs text-muted mb-1">{t.results.totalMarks}</p><p className="text-xl font-bold text-foreground">{totalMarks(result.subjects)}</p></div>
                  <div className="bg-surface rounded-xl p-4 text-center border border-border"><p className="text-xs text-muted mb-1">{t.results.obtained}</p><p className="text-xl font-bold text-primary">{totalObtained(result.subjects)}</p></div>
                  <div className="bg-primary/5 rounded-xl p-4 text-center border border-primary/20"><p className="text-xs text-muted mb-1">{t.results.percentage}</p><p className="text-xl font-bold text-primary">{result.percentage}%</p></div>
                  <div className="bg-surface rounded-xl p-4 text-center border border-border"><p className="text-xs text-muted mb-1">{t.results.division}</p><p className="text-lg font-bold text-foreground">{result.division}</p><p className="text-xs text-muted">{t.results.rank} {result.rank}</p></div>
                </div>
              </div>
              ...
              <div className="px-6 lg:px-10 py-5 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-border">
                <div className="flex items-center gap-3">
                  <span className="text-sm text-muted">{t.results.finalResult}</span>
                  <span className={`inline-flex items-center gap-1.5 text-sm font-bold px-4 py-1.5 rounded-full ${result.result === "Pass" ? "bg-green-100 text-green-700 border border-green-300" : "bg-red-100 text-red-700 border border-red-300"}`}>
                    {result.result === "Pass" ? t.results.passed : t.results.failed}
                  </span>
                </div>

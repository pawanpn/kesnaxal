    ...
    return (
      <section className="py-12 bg-surface">
        <div className="container-custom">
          <h2 className="text-2xl font-heading font-bold text-primary mb-8">Notice Board</h2>
      ...
      <div className="container-custom">
        ...
        <div className="space-y-3">
          {notices.map((notice) => (
            <div
              key={notice.id}
              className={`bg-white rounded-lg p-4 lg:p-5 border-l-4 ${priorityColor(notice.priority)} shadow-sm hover:shadow-md transition-shadow`}
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {isNew(notice.date) && (
                      <span className="shrink-0 bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">
                        New
                      </span>
                    )}
                    {notice.priority === "high" && (
                      <span className="shrink-0 bg-accent/10 text-accent text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">
                        Important
                      </span>
                    )}
                  </div>

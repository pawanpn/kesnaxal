...
export default function Hero() {
  ...
  return (
    <section className="relative w-full h-[500px] sm:h-[550px] lg:h-[600px] overflow-hidden bg-primary-dark">
      ...
      <button
        onClick={next}
        className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/40 text-white transition-colors"
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-colors ${
              index === current ? "bg-secondary" : "bg-white/50 hover:bg-white/80"
            }`}

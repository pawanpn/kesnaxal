      ...
      <section className="py-12 lg:py-16">
        <div className="container-custom">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            {[
              { label: "Open Positions", value: activeJobs.length.toString(), icon: "M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z", bg: "bg-primary/5" },
              { label: "Departments", value: [...new Set(activeJobs.map((j) => j.category))].length.toString(), icon: "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10", bg: "bg-secondary/10" },
              { label: "Total Vacancies", value: activeJobs.reduce((s, j) => s + j.vacancies, 0).toString(), icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z", bg: "bg-accent/5" },
              { label: "Location", value: "Kathmandu", icon: "M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z", bg: "bg-primary/5" },
            ].map((stat) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-lg font-heading font-bold text-primary">Available Positions</h2>
              </div>
              ...
              {activeJobs.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-2xl border border-border">
                  ...
                  <p className="text-muted text-sm">No open positions at the moment. Please check back later.</p>
                </div>
              ) : (
                activeJobs.map((job) => (
                  <div
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                          ...
                          <Badge variant={job.category === "Teaching" ? "primary" : job.category === "Administration" ? "accent" : "secondary"}>
                            {job.category}
                          </Badge>
                        </div>
                        <p className="text-xs text-muted mb-3">{job.level}</p>
                        <div className="flex flex-wrap items-center gap-4 text-xs text-muted">
                          <span className="flex items-center gap-1.5">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            Added: {new Date(job.addedOn).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}
                          </span>
                          <span className={`flex items-center gap-1.5 font-semibold ${
                            ...
                            Expires: {new Date(job.expiresOn).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}
                          </span>
                          <span className="flex items-center gap-1.5">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {selectedJob ? (
                <div className="bg-white rounded-2xl shadow-lg border border-primary/20 p-6 lg:sticky lg:top-24 animate-fadein">
                  ...
                  <div className="space-y-3 mb-6">
                    {[
                      ...
                      { label: "Job Level", value: selectedJob.level },
                      { label: "Experience", value: selectedJob.experience },
                      { label: "Salary", value: selectedJob.salary },
                      { label: "No. of Vacancies", value: selectedJob.vacancies.toString() },
                      { label: "Work Station", value: selectedJob.workstation },
                      { label: "Added On", value: new Date(selectedJob.addedOn).toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" }) },
                      { label: "Apply Before", value: new Date(selectedJob.expiresOn).toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" }) },
                    ].map((row) => (
                ...
                <div className="bg-surface rounded-2xl border border-border p-8 text-center lg:sticky lg:top-24">
                  ...
                  <p className="text-muted text-sm">
                    Click <strong>View Details</strong> on a job listing to see full information here.
                  </p>
                </div>
              )}
            </div>
